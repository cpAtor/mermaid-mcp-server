 ...
 */
function detectDiagramType(markup) {
    ...
    if (firstLine.startsWith("kanban"))
        return "kanban";
    if (firstLine.startsWith("architecture"))
        return "architecture";
    return "unknown";
}
 ...
 */
function validateMermaidSyntax(markup) {
    const trimmed = markup.trim();
    if (!trimmed) {
        return "Empty diagram markup";
    }
    const firstLine = trimmed.split("\n")[0].trim().toLowerCase();
    const recognised = KNOWN_DIAGRAM_KEYWORDS.some((kw) => firstLine.startsWith(kw));
    if (!recognised) {
        return (`Unrecognised diagram type. First line must start with a diagram keyword ` +
...
export function createServer() {
    ...
    registerAppTool(server, "renderMermaidDiagram", {
        title: "Render Mermaid Diagram",
        description: "Renders a Mermaid diagram with syntax validation, theme-aware styling, " +
    ...
    registerAppTool(server, "validateMermaidSyntax", {
